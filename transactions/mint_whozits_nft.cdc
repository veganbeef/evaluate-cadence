...
transaction(
    ...
    let minter: &WhozitsNFT.NFTMinter
    ...
    prepare(signer: AuthAccount) {
        self.mintingIDBefore = WhozitsNFT.totalSupply

        // borrow a reference to the NFTMinter resource in storage
        self.minter = signer.borrow<&WhozitsNFT.NFTMinter>(from: WhozitsNFT.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(WhozitsNFT.CollectionPublicPath)
    ...
    post {
        ...
        WhozitsNFT.totalSupply == self.mintingIDBefore + 1: "The total supply should have been increased by 1"
    }
}
