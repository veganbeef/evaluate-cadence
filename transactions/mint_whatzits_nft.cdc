...
/// This script uses the NFTMinter resource to mint a new NFT and deposit it in the recipient's collection
/// (It must be run with the account that has the minter resource stored in /storage/NFTMinter)
transaction(
    recipient: Address,
    name: String,
    description: String,
    ...
    let minter: &WhatzitsNFT.NFTMinter
    ...
    prepare(signer: AuthAccount) {
        self.mintingIDBefore = WhatzitsNFT.totalSupply

        // borrow a reference to the NFTMinter resource in storage
        self.minter = signer.borrow<&WhatzitsNFT.NFTMinter>(from: WhatzitsNFT.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(WhatzitsNFT.CollectionPublicPath)
    ...
    post {
        ...
        WhatzitsNFT.totalSupply == self.mintingIDBefore + 1: "The total supply should have been increased by 1"
    }
}
